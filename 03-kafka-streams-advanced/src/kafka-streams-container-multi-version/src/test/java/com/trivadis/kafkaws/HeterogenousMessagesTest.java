...
public class HeterogenousMessagesTest {
    ...
    @Test
    public void testAlertV2() throws ExecutionException, InterruptedException {
        com.trivadis.kafkaws.avro.v2.Alert alert = com.trivadis.kafkaws.avro.v2.Alert.newBuilder()
                .setAlerts(Arrays.asList("Alert Message 1 - v2", "Alert Message 2 - V2"))
                .setSeverity("HIGH")
                .build();
        com.trivadis.kafkaws.avro.v2.AlertSentEvent alertSentEvent = com.trivadis.kafkaws.avro.v2.AlertSentEvent.newBuilder()
                .setContext(Context.newBuilder().setId(UUID.randomUUID()).setWhen(Instant.now()).build())
                .setAlert(alert)
                .build();
        System.out.println(alertSentEvent);
